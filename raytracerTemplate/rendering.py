...
class Scene:
    ...
    def init_gl(self, ctx):
        ...
        texture_data = np.array([texture_coords[i]
                                for tri in triangles for i in tri])
        ...
        self.vao = self.ctx.vertex_array(
            self.shader, [(vbo, '2f 2f', *'vert tex_coords'.split())])
    ...
    def initialize_gl_texture(self):
        ...
        gl_texture = self.ctx.texture(
            (self.width, self.height), components=3, data=image_data, dtype='u1')
...
class RenderWindow:
    ...
    def __init__(self, scene):
        ...
        self.window = glfw.create_window(
            self.width, self.height, scene.scene_title, None, None)
        if not self.window:
            self.impl.shutdown()
            glfw.terminate()
            return
